...
const set = (layer, x, y, gid) => {
  if (inb(x, y)) L[layer][idx(x, y)] = gid;
};
const wall = (x, y) => {
  set('walls', x, y, WALL_GID);
  set('coll', x, y, 1);
};
const door = (x, y) => {
  set('walls', x, y, 0);
...
for (let y = 1; y < H - 1; y++) for (let x = 1; x < W - 1; x++) set('floor', x, y, floorGid(x, y));
...
const CAFE = {
  'cafe-seat-1': { x: 29, y: 14 },
  'cafe-seat-2': { x: 29, y: 16 }, // paired (same col, 2 apart)
  'cafe-seat-3': { x: 32, y: 14 },
  'cafe-seat-4': { x: 32, y: 16 },
  'cafe-stand-coffee': { x: 30, y: 20 },
  'cafe-stand-vending': { x: 33, y: 12 },
};
...
const walk = Array.from({ length: H }, (_, y) =>
  Array.from({ length: W }, (_, x) => L.coll[idx(x, y)] === 0),
);
const forceWalk = (t) => {
  if (inb(t.x, t.y)) walk[t.y][t.x] = true;
};
...
while (q.length) {
  ...
  for (const [nx, ny] of [
    ...
    if (inb(nx, ny) && !seen[ny][nx] && walk[ny][nx]) {
      seen[ny][nx] = true;
      q.push([nx, ny]);
    }
  }
...
const noApproach = Object.entries(SEATS).filter(
  ([, s]) =>
    ![
      [s.x, s.y + 1],
      [s.x, s.y - 1],
...
if (unreachable.length || noApproach.length) {
  ...
  if (noApproach.length)
    console.error('  no walkable approach:', noApproach.map(([n]) => n).join(', '));
  process.exit(1);
}
...
const seats = Object.keys(SEATS).length,
  cafe = Object.keys(CAFE).length;
console.log(
  `OK wrote ${path.relative(path.join(__dirname, '..'), OUT)} — ${W}x${H}, ${seats} desks, ${cafe} café spawns, zones: boardroom/cafeteria/holding`,
);
