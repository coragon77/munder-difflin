 ...
 *
 * THE SVG IS THE SOURCE OF TRUTH. archived_docs/logo.svg is authored here as pure
 ...
 *   archived_docs/logo.svg          source of truth (full bleed, ink border)
 *   archived_docs/logo.png          512  — site favicon, site header (dark), in-app
 *                                 toolbar + app favicon, README header
 *   archived_docs/logo-light.png    512  — site header on light theme (warm border)
 *   archived_docs/favicon-32.png     32  — native-size favicon, so browsers stop
 *                                 downsampling a 512px portrait into mush
 *   archived_docs/apple-touch-icon.png 180
...
function buildSvg(N, grid, frame, border) {
  ...
  <!-- Munder Difflin — the brand mark, and the source of truth for every raster
       in build/ and archived_docs/. Generated by tools/make-logo.cjs; edit that, not this.
...
write('archived_docs/logo.svg', Buffer.from(buildSvg(1024, grid, 'mark', BORDERS.ink)));
write('archived_docs/logo.png', rasterise(512, grid, 'mark', BORDERS.ink));
write('archived_docs/logo-light.png', rasterise(512, grid, 'mark', BORDERS.warm));
write('archived_docs/favicon-32.png', rasterise(32, grid, 'mark', BORDERS.ink));
write('archived_docs/apple-touch-icon.png', rasterise(180, grid, 'mark', BORDERS.ink));
