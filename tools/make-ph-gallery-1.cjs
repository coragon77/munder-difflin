...
const art = require(path.join(ROOT, 'test/load-ts.cjs'))(
...
const DESK = [58, 44, 32],
  DESK_T = [92, 70, 50];
const SCREEN = [26, 30, 26],
  SCREEN_ON = [126, 217, 87];
const BEZEL = [172, 162, 144],
  BEZEL_D = [128, 120, 106];
...
CAST.forEach((name, i) => {
  ...
  for (let sy = 0; sy < SH; sy++)
    for (let sx = 0; sx < SW; sx++) {
      const k = (sy * SW + sx) * 4;
      if (sp[k + 3] < 128) continue;
      const c = [sp[k], sp[k + 1], sp[k + 2]];
      for (let dy = 0; dy < SCALE; dy++)
        for (let dx = 0; dx < SCALE; dx++)
          px(ox + sx * SCALE + dx, SPRITE_TOP + sy * SCALE + dy, c);
    }
});
...
CAST.forEach((_, i) => {
  const ox = gap + i * (SW * SCALE + gap);
  const mw = 104,
    mx = ox + Math.floor((SW * SCALE - mw) / 2);
