...
export const GROK_COMMAND_GROUPS: CmdGroup[] = [
  {
    ...
    items: [
      { cmd: '/new', kind: 'slash', desc: 'Start a fresh session and clear the current context.' },
      {
        cmd: '/load',
        kind: 'slash',
        desc: 'Load a previous workspace session.',
        usage: '/load [workspace] [session]',
      },
      {
        cmd: '/compact',
        kind: 'slash',
        desc: 'Compact conversation history while retaining the important working context.',
        usage: '/compact keep the current task and next step',
      },
      {
        cmd: 'grok --resume <session-id>',
        kind: 'cli',
        desc: 'Resume an existing Grok session by ID or title.',
      },
    ],
  },
  {
    ...
    items: [
      ...
      },
      {
        cmd: '/always-approve',
        kind: 'slash',
        desc: 'Toggle automatic approval of tool executions.',
      },
      {
    ...
    items: [
      {
        cmd: '/rewind',
        kind: 'slash',
        desc: 'Rewind to a previous prompt and restore its file state.',
      },
