...
export interface RegistryAgent extends AgentMeta {
   ...
   *  vacation-request, the UI park button, the auto-park sweep (autoPark.ts).
   *  Set only via setPinned (the UI toggle's IPC); survives restarts and recalls
   *  through the `...prev` spread (like permissionMode). Never applies to the
   *  god agent. */
  pinned?: boolean;
...
/** MAIL FOLD budget (card agent-mail-queued-alongside-a--2026-08-19, god's
 *  size caveat): total folded-body characters before later pending messages
 *  fold as header-only + a drain pointer. ~4k tokens of inlined mail at most;
 *  a 40-message backlog cannot wall the contract or blow a context window,
 *  and no body is ever cut mid-sentence — overflow stays pending in the
 *  inbox, pointed at by the contract itself. */
const MAIL_FOLD_BODY_BUDGET = 16_000;
...
export class HiveManager {
  ...
  ensureHive(): void {
    ...
    // The registry-cwd writer (card agent-no-primitive-can-change--2026-08-21):
    // hive-retarget repoints a PARKED agent at an existing directory — the only
    // primitive that writes an agent's cwd. Same refresh policy.
    const retargetCli = join(root, 'bin', 'hive-retarget');
    writeFileSync(retargetCli, HIVE_RETARGET_CLI, 'utf8');
    if (process.platform !== 'win32') chmodSync(retargetCli, 0o755);
   ...
   * genuinely has no PTY, so the boot sweep, broadcast fan-out
  ...
  private identityText(meta: AgentMeta, integrationMode: IntegrationMode = 'god'): string {
    ...
    return [
      ...
      meta.isGod
        ? '- Monitor the team through the auto-injected LIVE ROSTER line (every prompt — reading fleet.json/registry.json directly is gate-REFUSED); full command reference in `COMMANDS.md`. `claude agents` does NOT list your hive siblings.'
        : '',
      '',
    ]
      .filter(Boolean)
  ...
  private injectedPrompt(
    ...
    // Lean also DEFERS analysis/design/data-model questions to role-marked
    // advisors (card agent-godline-defer-analysis-q-2026-08-20; root: Redmine
    // #3230 — god answered an analysis question inline and wrong).
    ...
    const godOwnsClause =
      integrationMode === 'lean'
        ? `conflict resolution, and translating worker reports into operator-readable form — and remain the sole scribe of board.md (the one exception: the standup clerk appends a single escalation line per anomalous standup — god stays the only AGENT scribe). LEAN-GOD POSTURE (integrationMode 'lean'): DEFAULT-DELEGATE mechanical-but-judgment work to the workers — they are Opus/pi-level and fork to lesser models themselves; do not pull such work into your own session. DEFER ANALYSIS TO ADVISORS: an analysis, design or data-model question is a CARD for an agent whose ROLE marks it as an advisor — resolve that agent from the LIVE ROSTER line, never from remembered names, and never answer it inline; when you catch yourself writing a design recommendation into a dispatch or an operator reply, that paragraph IS an advisor card (advisors carry bigger reasoning budgets — yours burns on orchestration; root: Redmine #3230). Do NOT re-verify worker-verified evidence — RECORD the reported hashes and gate results instead of re-running them. Your core role is the operator dialogue (talking/planning with the operator), task decomposition + dispatch contracts, conflict resolution, and translating worker reports into operator-readable form. VERIFIED-CLAIM RELAY: done-reports label each claim VERIFIED (check named) or INFERRED — transfer VERIFIED claims to the operator without scrutiny, but NEVER relay an INFERRED or unlabeled scale/infra claim to the operator without flagging it unverified (root incident #3216). ${delegatedClause}`
        : integrationMode === 'workers'
   ...
   *  MAIL FOLD (card agent-mail-queued-alongside-a--2026-08-19): a dispatch
   *  contract delivered DIRECT (adopt mode, providers with no typable clear)
   *  absorbs pending inbox mail first — see foldStagedContract. */
  private deliver(msg: HiveMessage, toId: string): void {
    ...
    // MAIL FOLD, direct half: a card-conversation contract publishes
    // ALREADY consolidated when it is the OLDEST pending message of its card
    // conversation (a later same-conversation message is an amendment — it
    // plain-delivers and the tick fold below merges it into the oldest). Any
    // fold failure falls back to plain delivery — a routing error here must
    ...
    try {
      ...
      // fall through: plain delivery beside pending mail; the tick fold consolidates
    }
  ...
  private doingCardOf(id: string): CardLike | null {
    const data = this.tasks() as { tasks?: CardLike[] };
    const cards = Array.isArray(data.tasks) ? data.tasks : [];
    return cards.find((c) => c?.assignee === id && c.status === 'doing') ?? null;
  }

  /** MAIL FOLD (card agent-mail-queued-alongside-a--2026-08-19): the doing
   *  card a message is the dispatch contract of, or null. The contract is
   *  identified structurally — a card-scoped conversation ('card-<id>') of a
   *  card that is doing AND assigned to the recipient — so god's amendments
   *  (same conversation) and hand-sent contracts both qualify. Pure read. */
  private dispatchContractCard(msg: HiveMessage, toId: string): CardLike | null {
    if (!msg.conversation?.startsWith('card-')) return null;
    const card = this.doingCardOf(toId);
    return card && `card-${card.id}` === msg.conversation ? card : null;
  }

  /** One folded message's section — the literal text an agent reads inside a
   *  dispatch contract that it did not read before (subject to the report).
   *  Bracketed as a FOREIGN message with sender, act (a folded 'query' must
   *  still read as waiting on an answer), subject, id — visibly separate mail,
   *  not lines of the contract itself (god's framing caveat, 2026-08-19). */
  private foldSection(m: HiveMessage): string {
    return [
      '--- MAIL FOLDED INTO THIS DISPATCH (queued beside it by another sender — handle it alongside this contract) ---',
      `From: ${m.from} | act: ${m.act} | Subject: ${m.subject} | id ${m.id}`,
      m.body,
      '--- END FOLDED MAIL ---',
    ].join('\n');
  }

  /** Header-only entry for a message beyond the fold budget (god's size
   *  caveat: never truncate a body mid-sentence — the full text stays pending
   *  where the drain finds it, and the contract points at it). */
  private foldOverflowHeader(m: HiveMessage): string {
    return [
      '--- MAIL QUEUED BESIDE THIS DISPATCH (beyond the fold budget — full text left pending; handle it too) ---',
      `From: ${m.from} | act: ${m.act} | Subject: ${m.subject} | id ${m.id}`,
      '--- END ---',
    ].join('\n');
  }

  /** All messages pending in a mail directory (top-level *.json), oldest
   *  first (deterministic — reviewer: readdir order must not decide which
   *  bodies fit the budget). Structurally invalid files are skipped: they are
   ...
   *  MAIL_FOLD_BODY_BUDGET chars last, then header-only entries. Only FULLY
  ...
  /** Is an OLDER same-card-conversation message already pending in the inbox?
  ...
  private releaseStagedAgent(
    ...
    for (const f of files) {
      try {
        stale.set(f, now - statSync(join(staged, f)).mtimeMs > MAIL_STAGE_TIMEOUT_MS);
      } catch {
        stale.set(f, false);
      }
    }
   ...
   *  half: among the staged files about to release, find the doing card's
   *  dispatch contract (oldest card-conversation message in the RELEASING
   *  set) and fold ALL other staged siblings — regardless of their own
   *  staleness (reviewer blocker: a stale contract releasing alone must not
   *  leave fresher amendments hidden in .staged) — plus any inbox-pending
   *  mail, into ONE combined budget. The enriched contract is rewritten
   ...
   *  unread card-conversation message of the agent's doing card; entries =
   *  every other pending inbox message + every staged leftover (any
   *  conversation), one budget. The anchor rewrite is atomic and keeps id +
   *  conversation, so a monitor keyed on inbox visibility sees one burst.
   ...
   *  standup triage — cards are the backlog channel, direct messages the
  ...
  rosterContext(forAgentId?: string, force = false): string | null {
    ...
    try {
      ...
      // activity and inbox anyway (not tokens or cost).
      ...
      if (unchanged) {
        ...
        return (
          ...
          mailStallLine +
          '.' +
          ' Detail beyond this line is a carded roster extension (direct fleet.json reads are gate-refused).'
        );
      }
...
**bin/ CLIs track the RUNNING app, not origin/main** — every \`$HIVE_ROOT/bin/hive-*\`
is rewritten from the main process's own bundle on every bootstrap and every
ledger write, so a CLI regenerated by hand reverts silently within minutes.
Never report a hand-regenerated CLI as live: a flag exists only once the app
runs a build containing it — commit, and the restart window lands it.
...
**Ask the human** (the card needs a decision or an action only the human can
take — this blocks the card and puts the ask on the office ASK ME board):

\`\`\`bash
"$HIVE_ROOT/bin/hive-card" ask <card-id> --q "Ship A or B?" --q "Who owns the rollout?"
\`\`\`

- ONE ASK PER ENTRY: every \`--q\` becomes its own \`humanQA\` entry — each
  carries its own answer, so a bundled numbered paragraph cannot be answered
  piecemeal and renders as a wall on the board. Keep each to a couple of
  sentences: the decision, the minimum context, your recommendation.
...
\`\`\`bash
"$HIVE_ROOT/bin/hive-card" prune-done            # dry run — lists what would go
"$HIVE_ROOT/bin/hive-card" prune-done --confirm  # actually removes them
\`\`\`

- Removes ONLY \`status: done\` cards, and only with \`--confirm\`; prints the
  count and the ids either way. Done cards stay on the board during the shift
  (the human reads the kanban by who-did-what) — prune once their outcome and
  doer are on board.md. Pruned cards remain recoverable via the hive's git
  history.
...
**Put a broken ledger back** (the recovery path when tasks.json is corrupt —
every other subcommand refuses to touch it, and hand-editing it is forbidden):

\`\`\`bash
"$HIVE_ROOT/bin/hive-card" restore --list              # which versions the hive git history has
"$HIVE_ROOT/bin/hive-card" restore --to <sha>          # dry run: what that version would restore
"$HIVE_ROOT/bin/hive-card" restore --to <sha> --confirm  # write it back
\`\`\`

- \`--list\` prints the newest commits touching tasks.json — short sha, date,
  card count (or UNUSABLE, so you never pick a version that is broken too).
- Without \`--to\`, the candidate is the newest committed version that parses.
  Dry run is the DEFAULT: it prints the candidate, the current card count (or
  UNREADABLE — the case restore exists for) and which card ids on disk the
  restore would lose. \`--confirm\` writes.
- The current file is copied to \`tasks.json.corrupt-<timestamp>\` beside it
  first, and the restore goes through the same lock + tempfile + rename as
  every other write. git is read ONLY (log/show) — the harness stays the only
  committer.

All subcommands validate before writing and refuse an unparseable ledger
instead of clobbering it — with \`restore\` as the way back. Errors explain
themselves on stderr (exit 1).`;
...
there); for non-god agents it stays the documented MANUAL FALLBACK only
when the CLIs are unavailable — same schema, same watcher.
...
**\`$HIVE_ROOT/bin/hive-retarget\` — repair a mis-registered cwd** (card
agent-no-primitive-can-change--2026-08-21). An agent's registered cwd is
otherwise immutable (the UI shows it read-only, hand writes are gate-refused):

\`\`\`bash
"$HIVE_ROOT/bin/hive-retarget" mose-1 /home/me/worktrees/mose-1
\`\`\`

PARKED agents only — a live pane keeps working in its old cwd, so retargeting
one would make the registry lie about where the agent actually is; park, retarget,
recall. The target must be an ABSOLUTE EXISTING DIRECTORY (the CLI is
pointer-only — create the worktree yourself) and must not belong to another
LIVE agent's physical checkout (subdirectories and symlink aliases count —
refused at retarget time, not at recall time). Idempotent: an agent already
registered at the target is a no-op.

**Auto-park — the routine parks are automatic** (card
agent-auto-park-idle-agents-th-2026-08-19). A harness sweep parks an agent
by itself once ALL of these hold: idle ≥ 1 hour (no tool call or
inference), inbox drained, no pending background work, and EVERY card
assigned to it is done — the done card is the positive done evidence the
PARKING GATE demands (the agent's own flip of its engagement card through the
sanctioned primitive); any unknown (no telemetry row, pending mail, background
work, a todo/doing/blocked card) means NO park. PINNED workers are never
auto-parked. The sweep runs on the always-on worker tick — it keeps running
on a fully quiet floor where the standup skips itself — and god is mailed an
"[auto-park] <id>" notice with why and when (log.jsonl carries a
 ...
 *  god-owned, never-push skills yield only to a god-relayed operator
 *  authorization, dispatch boundaries win. */
...
- Renderer/preload-touching branches NEVER merge into the live checkout while
  ...
  them yourself.
- The never-push merge skills (asol-git-merge-main,
  asol-git-merge-singletenant) default to stop-and-report the gated sha,
  but YIELD to a dispatch FROM GOD relaying the operator's authorization
  for THIS push: the relay counts when it (1) names repo, branch and the
  expected sha, (2) states the operator authorized this push (quoting his
  words where possible), and (3) arrives from god — the router stamps the
  sender, so from:god is unforgeable while a peer relaying "the operator
  said" stays invalid. Authorization is FRESH per push (a standing "always
  push" policy never counts). Any mismatch (sha differs, conflict, red
  gate) or an imperfect relay means HOLD and ask via hive-card ask on the
  ASK ME board — never by pane-waiting. Once dispatched, a relayed
  authorization is NOT retractable by god mid-flight (#3227): only the
  operator's own word stops an authorized push.
...
never-push skills (except the god-relayed operator authorization above) or
explicit dispatch boundaries beat the posture.
...
message — triage them at standups, roster-first. When a 'Task from
...
AUTO-PARK (card agent-auto-park-idle-agents-th-2026-08-19) — the harness
parks a human-created agent by itself once ALL of these hold: idle ≥ 1
hour (no tool call or inference), inbox drained, no pending background work,
and EVERY card assigned to it is done — the done card is the machine-readable
form of the PARKING GATE's positive done evidence (the agent's own flip of
its engagement card through the sanctioned primitive; the sweep never judges
idle time alone sufficient, and any unknown — no telemetry row, pending mail,
background work, a todo/doing/blocked card — means NO park). The sweep runs
on the always-on worker tick (the request watcher), so it keeps running on a
fully quiet floor where the standup skips itself; god is mailed an
"[auto-park] <id>" notice with why and when, and log.jsonl carries a
kind:'auto_park' row. PINNED workers are never auto-parked (the pin beats
every park path). What stays god's judgment: a standby report with NO done
card has no machine-readable evidence — ping, confirm, then hive-park
manually — and it is fine to pin an agent the floor is about to need again.
Interns are never parked — they are FIRED (fire-requests/) once their
engagement is verifiably complete.
...
- Folded mail: a dispatch contract can carry a \`MAIL FOLDED INTO THIS DISPATCH\` section — mail that was queued beside your dispatch is folded INTO the contract (originals already archived to \`inbox/.done/\`). There is nothing beside it to drain: the folded section IS that mail, handle it as part of the dispatch.
...
function readLedger() {
  ...
  // The recovery instruction is a COMMAND, not "go hand-fix it": hand-editing
  // tasks.json is what the ledger gate refuses, with no override flag.
  catch (_) { fail('tasks.json is not parseable JSON — refusing to write. Recover it: hive-card restore --list, then hive-card restore --to <sha> --confirm.'); }
...
function cmdUpdate(argv) {
  ...
  // HARD HOLD (card agent-make-the-paused-hold-har-2026-08-19): paused is the
  // operator's hold — setting or clearing it via this CLI is GOD-ONLY, and
  // god's own briefing binds that to the operator's explicit order (godLine).
  // The operator's own toggle is the UI (tasks tab / office), which writes
  // through the main process, never this CLI. callerIsGod (the registry
  // godId check cmdAdd already uses) is reused — one identity check, not two.
  if ((pausedFlag || resumeFlag) && !callerIsGod()) {
    fail('refused: card "' + cardId + '" — paused is the operator\\'s hold: ONLY god may set or ' +
      'clear it, and only on the operator\\'s explicit instruction (you are "' +
...
// Ask the human (card agent-hive-card-ask-prune-done-2026-08-19): append the
// asks to the card's humanQA and block the card. god wrote both by hand into
// tasks.json until now — the one file its own instructions forbid hand-editing.
// Three rules that were folklore and are now behaviour:
//   ONE ASK PER ENTRY — every --q is its own entry, never a joined paragraph;
//     an entry is the unit of ANSWERING (it carries its own "a").
//   ORDER — the ASK ME board (openQuestion in the renderer's TasksKanban.tsx)
//     walks humanQA BACKWARDS and surfaces the LAST unanswered, undismissed
//     entry. So entries land REVERSED: --q A --q B --q C writes [C, B, A] and
//     the operator is asked A, then B, then C. god used to hand-write that
//     reversal; the CLI owns it now.
//   HISTORY IS APPEND-ONLY — existing entries and their answers are never
//     rewritten, reordered or dropped; they document the card's decisions.
// The blocked flip is a documented side effect of asking, and the only status
// write here (hive-dispatch owns todo->doing, hive-card status owns the rest).
function cmdAsk(argv) {
  if (argv.length < 1 || argv[0].indexOf('--') === 0) usage();
  var cardId = argv[0];
  // Hand-rolled parse: parseFlags keys by name, so a repeated --q would
  // collapse to the last one — exactly the bundling this command forbids.
  var qs = [];
  for (var i = 1; i < argv.length; i++) {
    var a = argv[i];
    if (a.indexOf('--') !== 0) fail('unexpected argument: ' + a + ' (ask takes <id> then --q <text>).');
    a = a.slice(2);
    var v;
    var eq = a.indexOf('=');
    if (eq >= 0) { v = a.slice(eq + 1); a = a.slice(0, eq); }
    else { v = argv[++i]; }
    if (a !== 'q') fail('unknown flag --' + a + ' (ask takes --q <text>, repeatable).');
    if (v === undefined) fail('missing value for --q');
    if (!v.trim()) fail('--q must be non-empty when given.');
    qs.push(v.trim());
  }
  if (!qs.length) fail('give at least one --q <text> (one ask per entry — repeat --q for several).');
  ...
  process.stdout.write(
    cardId + ' -> blocked, ' + qs.length + ' ask' + (qs.length === 1 ? '' : 's') +
    ' appended (' + total + ' humanQA entries on the card)\\n');
}

// Shift-close ledger hygiene (same card): done cards stay on the board all
// shift — the human reads the kanban by who-did-what — then go in ONE sweep.
// Dry run is the DEFAULT (the write needs --confirm) and only status 'done' is
// ever removed; pruned cards stay recoverable in the hive's git history.
function cmdPruneDone(argv) {
  var confirm = false;
  var dryFlag = false;
  for (var i = 0; i < argv.length; i++) {
    if (argv[i] === '--dry-run') dryFlag = true;
    else if (argv[i] === '--confirm') confirm = true;
    else fail('unexpected argument: ' + argv[i] + ' (prune-done takes --dry-run or --confirm).');
  }
  if (confirm && dryFlag) fail('give either --dry-run or --confirm, not both.');
  var dry = !confirm;
  var ids = [];
  // The filter runs INSIDE the lock on a fresh read: a landing stamp written
  // between an earlier dry run and this write is never clobbered.
  withLock(function () {
    const data = readLedger();
    ids = data.tasks.filter((t) => t && t.status === 'done').map((t) => t.id);
    if (dry || !ids.length) return;
    data.tasks = data.tasks.filter((t) => !(t && t.status === 'done'));
    writeLedger(data);
  });
  process.stdout.write(
    'prune-done: ' + ids.length + ' done card' + (ids.length === 1 ? '' : 's') +
    (dry ? ' would be removed (dry run — pass --confirm to write)' : ' removed') +
    (ids.length ? '\\n' + ids.join('\\n') : '') + '\\n');
}

// Restore a known-good tasks.json (card agent-hive-card-restore-bound--2026-08-19).
// Every other subcommand — and hive-dispatch — refuses an unparseable ledger,
// and the ledger gate refuses god's direct writes with no override flag: the
// CLI's own "fix it first" instruction was the one operation left forbidden.
// So recovery is a PRIMITIVE, not an escape hatch. The hive directory is a git
// repo whose history already backs prune recovery ("pruned cards remain
// recoverable via the hive git history") — this reads a past tasks.json out of
// that history and writes it back through the SAME lock + tempfile + rename
// every other write uses. git here is READ-ONLY (log/show, no checkout, no
// index): the main process stays the only committer.
// It exists FOR the corrupt case, so it never reads the current ledger as a
// precondition — the CANDIDATE is what gets validated, and one that does not
// parse is refused. Dry run is the default; --confirm writes and backs the
// current file up beside the target first, so nothing is lost.
const RESTORE_LIST_N = 20;
...
function assertRepo() {
  try { execFileSync('git', ['rev-parse', '--git-dir'], { cwd: root, stdio: 'ignore' }); }
    ...
    fail('the hive at ' + root + ' is not a git repo (or git is unavailable) — restore reads tasks.json out of the hive history, so there is nothing to restore from.');
  }
}

function ledgerCommits(limit) {
  var out = gitOut(['log', '--format=%H%x09%cI', '-n', String(limit), '--', 'tasks.json']);
  var rows = [];
  var lines = out.split('\\n');
  for (var i = 0; i < lines.length; i++) {
    if (!lines[i].trim()) continue;
    var parts = lines[i].split('\\t');
    rows.push({ sha: parts[0], date: parts[1] || '' });
  }
  return rows;
}

// The candidate as it was at <sha>: blob read only, validated like readLedger
// validates the live file. Never throws — the caller reports why it is unusable.
function ledgerAt(sha) {
  var raw;
  try { raw = gitOut(['show', sha + ':./tasks.json']); }
  catch (_) { return { ok: false, why: 'no tasks.json in that commit' }; }
  var data;
  try { data = JSON.parse(raw); }
  catch (_) { return { ok: false, why: 'that version is not parseable JSON either' }; }
  if (!data || typeof data !== 'object' || !Array.isArray(data.tasks)) {
    return { ok: false, why: 'that version has an unexpected shape (want {"tasks": [...]})' };
  }
  return { ok: true, data: data, count: data.tasks.length };
}

function shortSha(sha) { return sha.slice(0, 8); }

// Best-effort read of the CURRENT ledger for the delta line. NOT a precondition:
// unreadable is the normal case here, and returns null rather than failing.
function currentTasksOrNull() {
  try {
    var d = JSON.parse(fs.readFileSync(ledgerPath, 'utf8'));
    return d && Array.isArray(d.tasks) ? d.tasks : null;
  } catch (_) { return null; }
}
...
function cmdRestore(argv) {
  ...
  for (var i = 0; i < argv.length; i++) {
    var a = argv[i];
    if (a === '--list') list = true;
    else if (a === '--dry-run') dryFlag = true;
    else if (a === '--confirm') confirm = true;
    else if (a === '--to' || a.indexOf('--to=') === 0) {
      var v = a.indexOf('=') > 0 ? a.slice(5) : argv[++i];
      if (v === undefined || !String(v).trim()) fail('--to needs a commit sha (see: hive-card restore --list).');
      to = String(v).trim();
    } else fail('unexpected argument: ' + a + ' (restore takes --list, --to <sha>, --dry-run, --confirm).');
  }
  if (confirm && dryFlag) fail('give either --dry-run or --confirm, not both.');
  if (list && (confirm || dryFlag || to)) fail('--list is read-only — give it on its own.');
  assertRepo();
  var rows = ledgerCommits(RESTORE_LIST_N);

  if (list) {
    var out = ['restore candidates — newest ' + rows.length + ' commit' + (rows.length === 1 ? '' : 's') + ' touching tasks.json:'];
    for (var j = 0; j < rows.length; j++) {
      var at = ledgerAt(rows[j].sha);
      out.push(
        '  ' + shortSha(rows[j].sha) + '  ' + rows[j].date + '  ' +
        (at.ok ? at.count + ' card' + (at.count === 1 ? '' : 's') : 'UNUSABLE (' + at.why + ')'),
      );
    }
    if (!rows.length) out.push('  (none — no commit in the hive history touches tasks.json)');
    process.stdout.write(out.join('\\n') + '\\n');
    return;
  }
  ...
  var sha;
  if (to) {
    try { sha = gitOut(['rev-parse', '--verify', to + '^{commit}']).trim(); }
    catch (_) { fail('no commit "' + to + '" in the hive history (see: hive-card restore --list).'); }
  } else {
    // Default: the newest version that would actually be accepted — defaulting
    // to a candidate restore would refuse anyway is no default at all.
    for (var k = 0; k < rows.length && !sha; k++) if (ledgerAt(rows[k].sha).ok) sha = rows[k].sha;
    if (!sha) {
      fail(rows.length
        ? 'none of the newest ' + rows.length + ' committed tasks.json versions is usable — pass --to <sha> with an older one.'
        : 'no commit in the hive history touches tasks.json — nothing to restore from.');
    }
  }
  var cand = ledgerAt(sha);
  if (!cand.ok) fail('refusing to restore ' + shortSha(sha) + ': ' + cand.why + ' — pick another with: hive-card restore --list.');

  var cur = currentTasksOrNull();
  var lost = cur
    ? cur.filter(function (t) { return t && !cand.data.tasks.some(function (x) { return x && x.id === t.id; }); }).map(function (t) { return t.id; })
    : [];
  var lines = [
    '  candidate  ' + shortSha(sha) + '  ' + (rows.filter(function (r) { return r.sha === sha; })[0] || { date: '' }).date + '  ' + cand.count + ' card' + (cand.count === 1 ? '' : 's'),
    '  current    ' + (cur ? cur.length + ' cards on disk' : 'UNREADABLE — which is what restore is for'),
  ];
  if (cur) {
    lines.push('  delta      ' + lost.length + ' card' + (lost.length === 1 ? '' : 's') + ' on disk would be lost' + (lost.length ? ': ' + lost.join(', ') : ''));
  }

  if (!confirm) {
    process.stdout.write(
      ['restore (dry run — pass --confirm to write):'].concat(lines).join('\\n') +
      '\\nnothing written.\\n');
    return;
  }
  ...
  withLock(function () {
    if (fs.existsSync(ledgerPath)) {
      backup = ledgerPath + '.corrupt-' + new Date().toISOString().replace(/[:.]/g, '-');
      fs.copyFileSync(ledgerPath, backup);
    }
    writeLedger(cand.data);
  });
  process.stdout.write(
    ['restored tasks.json from ' + shortSha(sha) + ' (' + cand.count + ' cards)'].concat(lines.slice(1)).join('\\n') +
    '\\n  backup     ' + (backup || 'none — there was no tasks.json to back up') + '\\n');
}
...
try {
  ...
  else if (cmd === 'ask') cmdAsk(process.argv.slice(3));
...
// ─── hive-retarget (written to <hive>/bin/hive-retarget) ──────────────────────
// The ONLY writer of an agent's registry cwd (card agent-no-primitive-can-
// change--2026-08-21). Before it, NO primitive could change a registered cwd:
// the shared-state gate refuses every hand path and no CLI wrote it, so a
// mis-registered agent (both advisors seated on the physical checkout instead
// of their existing worktrees) stayed mis-registered forever — and a recall
// re-enters the recorded cwd, so it hit the one-agent-per-directory refusal
// with no way out. Direct-write like hive-card (fresh read + atomic rename;
// the change rides the next main-process `git add -A` hive commit), guards
// decided on the card:
//  (a) PARKED-ONLY — a live pane keeps working in its old cwd; a registry
//      rewrite under it would make the registry lie about where the live
//      agent actually is and the spawn guard would stop seeing the real
//      conflict. Escape: hive-park, hive-retarget, hive-recall.
//  (b) target must be an ABSOLUTE EXISTING DIRECTORY, and must not belong to
//      another LIVE agent's PHYSICAL CHECKOUT — compared by the same identity
//      spawnAgentCore uses (git rev-parse --show-toplevel + realpath), so a
//      subdirectory or symlink alias of a live seat is refused HERE, not at
...
const root = process.env.HIVE_ROOT;
if (!root) {
  process.stderr.write('hive-retarget: HIVE_ROOT must be set — run this from inside a hive agent pane.\\n');
  process.exit(1);
}
${ASSERT_LIVE_HIVE}
assertLiveHive(root);
...
function usage() {
  fail('usage: hive-retarget <agentId> <newCwd>  — repoint a PARKED agent at an existing directory (pointer-only: create the worktree yourself first).');
}

// The PHYSICAL CHECKOUT a seat belongs to — the same identity physicalCheckout()
// compares at spawn time (src/main/git.ts, card agent-one-agent-per-directory--
// 2026-08-19): the worktree root inside a linked worktree (each worktree is its
// own checkout), the repo root inside a normal checkout (a subdirectory seat
// occupies the whole checkout), realpath for non-repos (symlink aliases
// collapse), the input unchanged when the path is unreachable.
function physicalCheckout(seat) {
  let top = '';
  try {
    top = execFileSync(
      'git', ['-C', seat, 'rev-parse', '--path-format=absolute', '--show-toplevel'],
      { encoding: 'utf8', stdio: ['ignore', 'pipe', 'ignore'] },
    ).trim();
  } catch (_) { /* not a repo, or no git — fall through */ }
  if (top) {
    try { return fs.realpathSync(top); } catch (_) { return top; }
  }
  try { return fs.realpathSync(seat); } catch (_) { return seat; }
}
...
function withLock(fn) {
  ...
  fail('could not acquire the registry.json lock — another writer seems stuck.');
}

function readRegistry() {
  try { return JSON.parse(fs.readFileSync(registryPath, 'utf8')); }
  catch (_) { fail('registry.json is not readable — cannot retarget.'); }
}

function refuseUnparked(entry, id) {
  if (entry.retired)
    fail('"' + id + '" was fired — retired agents are not retargeted (reinstate first if it should ever come back).');
  fail('"' + id + '" is not parked — a live pane keeps working in its old cwd and the registry must keep telling the truth about where the agent actually is (the one-agent-per-directory guard would miss the real conflict). Park it first ($HIVE_ROOT/bin/hive-park ' + id + '), retarget, then hive-recall.');
}

function main() {
  const pos = [];
  for (const a of process.argv.slice(2)) {
    if (a.indexOf('--') === 0) fail('unknown flag ' + a + ' (usage: hive-retarget <agentId> <newCwd>)');
    pos.push(a);
  }
  if (pos.length !== 2) usage();
  const agentId = pos[0].trim();
  const rawCwd = pos[1].trim();
  if (!agentId || !rawCwd) usage();

  const reg = readRegistry();
  const agents = reg.agents || {};
  if (!Object.prototype.hasOwnProperty.call(agents, agentId))
    fail('no agent "' + agentId + '" in registry.json — hive-roster list shows every id.');
  const entry = agents[agentId];
  if (!entry.vacation) refuseUnparked(entry, agentId);

  if (!path.isAbsolute(rawCwd)) fail('"' + rawCwd + '" is not an absolute path.');
  const newCwd = path.resolve(rawCwd);
  let st;
  try { st = fs.statSync(newCwd); }
  catch (_) {
    fail('"' + newCwd + '" does not exist — hive-retarget is pointer-only: create the directory (or git worktree) first, then point at it.');
  }
  if (!st.isDirectory()) fail('"' + newCwd + '" is not a directory.');

  if (entry.cwd && path.resolve(entry.cwd) === newCwd) {
    process.stdout.write('hive-retarget: ' + agentId + ' is already registered at ' + newCwd + ' — nothing to do.\\n');
    return;
  }

  // One agent per directory, enforced AT RETARGET TIME with the spawn guard's
  // own seat identity: a subdirectory or symlink alias of a live agent's
  // checkout is the same conflict the recall would hit — refuse it here,
  // with the holder named, instead of moving the failure to recall time.
  const target = physicalCheckout(newCwd);
  for (const id of Object.keys(agents)) {
    if (id === agentId || id === reg.godId) continue;
    const a = agents[id];
    if (!a || a.isGod || a.archived || a.vacation || a.retired) continue; // off the floor
    if (typeof a.cwd !== 'string' || !a.cwd) continue;
    if (physicalCheckout(path.resolve(a.cwd)) === target)
      fail('one agent per directory — ' + id + ' works in the physical checkout ' + target + ', so pointing "' + agentId + '" there would hand its next recall the same refusal. Give "' + agentId + '" its own worktree, or park ' + id + ' first.');
  }

  withLock(function () {
    // FRESH read inside the lock: the main process re-reads registry.json on
    // every mutation, and a stale pre-lock copy would clobber a concurrent
    // write (session stamps, vacation flags) with the old file.
    const fresh = readRegistry();
    const e = fresh.agents && fresh.agents[agentId];
    if (!e) fail('no agent "' + agentId + '" in registry.json (changed under the lock) — re-run.');
    if (!e.vacation) refuseUnparked(e, agentId); // recalled mid-flight — re-check
    const old = e.cwd || '(unset)';
    e.cwd = newCwd;
    e.cwdValid = true;
    ...
    process.stdout.write(
      'retargeted ' + agentId + ' (' + (entry.name || agentId) + '): ' + old + ' -> ' + newCwd + '\\n' +
      'recall it with $HIVE_ROOT/bin/hive-recall ' + agentId + '\\n');
  });
}
try { main(); }
catch (e) {
  process.stderr.write('hive-retarget: ' + (e && e.message ? e.message : String(e)) + '\\n');
  process.exit(1);
}
`;
