...
export interface HiveTask {
   ...
   *  cards are assigned). Absent = not paused; the →doing flip clears it
   *  (auto-resume) in every writer. */
  paused?: boolean;
}
...
function cmdStatus(argv) {
  ...
  withLock(function () {
    ...
    card.status = next;
    if (next === 'doing') delete card.paused; // auto-resume: a resumed card carries no on-hold flag into doing
    writeLedger(data);
  });
