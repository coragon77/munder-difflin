...
const STOPWORDS = new Set(
  (
    ...
    'would should about into over than then there here so if no yes'
  ).split(' '),
);
...
function extractText(input) {
  ...
  if (typeof inlineText === 'string' && inlineText.length) {
    return {
      text: inlineText,
      title: title || deriveTitle(inlineText) || source,
      extractor: 'inline@1',
      mime: 'text/plain',
    };
  }
  ...
  if (modality === 'image') {
    ...
    return {
      text: parts.join('\n'),
      title: title || source,
      extractor: 'image-meta@1',
      mime: mimeFor(srcPath),
    };
  }
  ...
  if (modality === 'pdf') {
    ...
    if (out != null && out.trim()) {
      return {
        text: out,
        title: title || deriveTitle(out) || source,
  ...
  if (srcPath && fs.existsSync(srcPath)) {
    ...
    return {
      text: raw,
      title: title || deriveTitle(raw) || source,
      extractor: 'text-utf8@1',
      mime: mimeFor(srcPath),
    };
  }
...
function tryPdfToText(srcPath) {
  ...
  try {
    const res = spawnSync('pdftotext', ['-q', srcPath, '-'], {
      encoding: 'utf8',
      timeout: 20000,
...
function ingest(kgRoot, input = {}) {
  ...
  const lines = chunks.map((c, i) =>
    JSON.stringify({ docId, title, source, modality, chunkIdx: i, text: c }),
  );
...
function list(kgRoot) {
  ...
  for (const id of fs.readdirSync(docsDir)) {
    try {
      out.push(JSON.parse(fs.readFileSync(path.join(docsDir, id, 'meta.json'), 'utf8')));
    } catch {
      /* skip */
    }
  }
...
function getDoc(kgRoot, docId) {
  ...
  let meta = null;
  try {
    meta = JSON.parse(fs.readFileSync(path.join(docDir, 'meta.json'), 'utf8'));
  } catch {
    /* none */
  }
  if (!meta) return null;
  let text = '';
  try {
    text = fs.readFileSync(path.join(docDir, 'text.md'), 'utf8');
  } catch {
    /* none */
  }
  return { meta, text };
