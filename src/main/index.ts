...
async function processSpawnRequest(filePath: string): Promise<void> {
  ...
  try {
    ...
    const suffix = persistent
      ? `\n\n[CAPABILITIES] Before you start, consult your capability catalog — run the \`/capabilities\` skill (or read \`$AGENT_DIR/.claude/skills/capabilities/SKILL.md\`). It lists your temporal date-range skills (\`/today\`, \`/last30Days\`, \`/lastQuarter\`, …) and the integrations available to you (reached via the loopback broker) and how to call each. For any time-scoped work, resolve the dates with those skills instead of computing them by hand.\n\n[INTERN HIRE] You are a STANDING member of the floor (an INTERN hired by god) — you are NOT released when this task completes. When finished, send ONE outbox message to god with "act":"done" and a short result summary, then await further instructions in your inbox. On pushing, the dispatch contract and the house integrationMode decide: with integrationMode 'workers' or 'lean' you merge and push your own branches once the gates are green; otherwise god is the sole integrator — do not push.`
      : `\n\n[CAPABILITIES] Before you start, consult your capability catalog — run the \`/capabilities\` skill (or read \`$AGENT_DIR/.claude/skills/capabilities/SKILL.md\`). It lists your temporal date-range skills (\`/today\`, \`/last30Days\`, \`/lastQuarter\`, …) and the integrations available to you (reached via the loopback broker) and how to call each. For any time-scoped work, resolve the dates with those skills instead of computing them by hand.\n\n[WORKER COMPLETION] When finished, signal done by sending ONE outbox message to god with "act":"done" and a short result summary — that releases this ephemeral worker (terminal closed; your branch is handed to god). On pushing, the dispatch contract and the house integrationMode decide: with integrationMode 'workers' or 'lean' you merge and push your own branches once the gates are green; otherwise god is the sole integrator — do not push.`;
    hive.send(
      {
        to: workerId,
  ...
  return parkAgentCore(
    {
      ...
      killPty: (ptyId) => ptyManager.kill(ptyId),
      teardownPty,
      setVacation: (id, v) => hive.setVacation(id, v),
      appendLog: (e) => hive.appendLog(e),
      notifyVacationed: (e) => liveWebContents()?.send('hive:agentVacationed', e),
      log: (m) => console.log(m),
  ...
  return recallAgentCore(
    {
      hiveEnabled: () => hive.enabled(),
      registry: () => hive.registry(),
      isOnVacation: (id) => hive.isOnVacation(id),
      ptyForAgent,
      recipe: rosterRecipe(agentId),
...
async function processVacationRequest(filePath: string): Promise<void> {
  ...
  // Parse/verb/id resolution lives in vacationFlow.vacationRequestTarget
  // (pinned there) — `id` beside `agentId`, verb "recall" case-insensitive.
  const plan = vacationRequestTarget(raw);
  if (!plan.ok) {
    fail(plan.error);
    return;
  }
...
function autoParkSweep(): void {
  ...
  for (const [id, a] of Object.entries(reg.agents)) {
    ...
    const candidate: AutoParkCandidate = {
      ...
      pendingBackgroundWork: pendingWork.countFor(id),
      // undefined when UNREADABLE — unknown ≠ drained (review finding 3).
