...
> = {
  ...
  officeTheme: {
    tier: 'soft',
    type: 'string',
    values: ['office', 'friends', 'brooklyn99', 'siliconvalley', 'got', 'hogwarts'],
  },
...
function resolveAgent(target: string, reg: Registry): ResolvedAgent | { error: string } {
  ...
  const partial = entries.filter(
    ([id, m]) =>
      !m.archived && (id.toLowerCase().includes(t) || (m.name || '').toLowerCase().includes(t)),
  );
  if (partial.length === 1) return mk(partial[0][0], partial[0][1]);
  if (partial.length > 1)
    return { error: `several agents match "${target}" — be more specific or say an id` };
...
function execPing(deps: RealtimeActionDeps, a: Record<string, unknown>): ActionResult {
  ...
  deps.hiveSend(
    { to: r.id, act: 'inform', subject: 'Voice ping from Michael', body: message },
    VOICE_ACTOR,
  );
  ...
  const msg = deps.hiveSend(
    {
      to: r.id,
      act: 'request',
      subject: `Voice dispatch: ${objective.slice(0, 60)}`,
      body,
      requires_reply: true,
    },
    VOICE_ACTOR,
...
function execCreateTask(deps: RealtimeActionDeps, a: Record<string, unknown>): ActionResult {
  ...
  return {
    ok: true,
    spoken: `Created task "${title}"${card.assignee ? `, assigned to ${card.assignee}` : ''}.`,
  };
}
...
function execAutoDelivery(deps: RealtimeActionDeps, a: Record<string, unknown>): ActionResult {
  ...
  const raw = norm(
    str(a.state) ||
      str(a.action) ||
      (a.paused === true ? 'pause' : a.paused === false ? 'resume' : ''),
  );
...
function execGateTool(deps: RealtimeActionDeps, a: Record<string, unknown>): ActionResult {
  ...
  if (!toolName || !/^[A-Za-z][A-Za-z0-9_-]{0,63}$/.test(toolName)) {
    return {
      ok: false,
      spoken: 'Which tool should I gate? Give me its exact name, like Bash or WebFetch.',
    };
  }
...
function buildKill(deps: RealtimeActionDeps, r: ResolvedAgent): () => Promise<string> {
  return async () => {
    ...
    return res.ok
      ? `Killed ${r.name}.`
      : `Couldn't kill ${r.name}: ${res.error || 'unknown error'}.`;
  };
}
...
function proposeDestructive(
  ...
  if (spec.agentTargeted) {
    ...
    if (isMassTarget(rawTarget))
      return {
        ok: false,
        spoken: `${verb} on all agents at once is voice-forbidden. Do it agent by agent, or use the UI.`,
      };
    ...
    if (r.isGod && verb !== 'clear_context')
      return {
        ok: false,
        spoken: `${verb} on the god orchestrator is voice-forbidden. That has to be done in the UI.`,
      };
      ...
      verb === 'kill'
        ...
        : verb === 'pause'
          ...
          : verb === 'halt'
            ? buildHalt(deps, r)
            : verb === 'clear_context'
              ? buildClearContext(deps, r)
              : buildArchive(deps, r);
    const breaker = deps.controlSnapshot(r.id);
    const note = breaker?.halted
      ? ' (note: already halted)'
      : breaker?.paused
        ? ' (note: already paused)'
        : '';
  ...
  if (verb === 'spawn') {
    ...
    const name =
      str(a.name) || (role ? role.replace(/\b\w/g, (c) => c.toUpperCase()) : provider) || 'Worker';
    ...
    if (!cwd)
      return { ok: false, spoken: 'I need a working directory to hire into — none is configured.' };
    ...
    pending = {
      ...
      targetLabel: name,
      createdAt: Date.now(),
      commit: buildSpawn(deps, spec2, `${name} on ${provider}`),
    };
    return {
      ok: true,
      needsConfirm: true,
  ...
  if (verb === 'edit_schedule') {
    ...
    const m =
      ...
      missions.find(
        (x) => (x.label || '').toLowerCase().includes(ref) || x.id.toLowerCase().includes(ref),
      );
    if (!m)
      return {
        ok: false,
        spoken: ref
          ? `I couldn't find a schedule matching "${str(a.label) || ref}".`
          : 'Which schedule should I edit?',
      };
    const raw = norm(str(a.action) || str(a.op));
    const action: 'enable' | 'disable' | 'delete' =
      raw.includes('delete') || raw.includes('remove')
        ? 'delete'
        : raw.includes('disable') || raw.includes('off') || raw.includes('pause')
          ? 'disable'
          : 'enable';
    pending = {
      verb,
  ...
  if (verb === 'update_setting') {
    ...
    if (!policy) {
      return {
        ok: false,
        spoken: `The "${key}" setting can't be changed by voice — use the Settings screen for that one.`,
      };
    }
    ...
    if (policy.type === 'boolean') {
      ...
      if (policy.max !== undefined && value > policy.max)
        return { ok: false, spoken: `${key} can't go above ${policy.max}.` };
    ...
    const describe = (v: unknown): string =>
      typeof v === 'boolean' ? (v ? 'on' : 'off') : String(v ?? 'unset');
    ...
    const applyNow = (): string => {
      ...
      attribute(deps, 'update_setting', key, {
        action: `${describe(oldValue)} → ${describe(value)}`,
      });
...
function runAction(
  ...
  if (!deps.hiveEnabled())
    return { ok: false, spoken: "The hive is not configured, so I can't take that action." };
  const spec = VERBS[verb];
...
export function registerRealtimeActionIpc(deps: RealtimeActionDeps): void {
  ...
  ipcMain.handle('realtime:action:confirm', async (_evt, payload: unknown) => {
    ...
    if (!cur)
      return { ok: false, spoken: "There's nothing waiting to confirm." } satisfies ActionResult;
  ...
  ipcMain.handle('realtime:action:cancel', async () => {
    ...
    return {
      ok: true,
      spoken: had ? `Cancelled the ${had.verb}.` : 'Nothing to cancel.',
    } satisfies ActionResult;
  });
}
