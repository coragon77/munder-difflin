 ...
 *
 * SERIALIZED AS A SET: hive.ts serializes these functions verbatim
 ...
 * exercise. orientationRoots is the detection half; orientationBlock CALLS
 * IT BY NAME, so hive.ts must serialize BOTH, orientationRoots first under
 * that exact name — and it carries a load-time tripwire that fails loudly
 * if a bundler ever renames that inner reference. No imports, no other
 * module-scope references: the pair is self-contained together. FAIL OPEN is
 * non-negotiable: on ANY error orientationBlock returns '' and the dispatch
 * proceeds with the body unmodified — a broken injector must never block a
 * dispatch (Card B backstops). Probes are the injected (path) => boolean —
 * existsSync at the CLI call sites, fake sets in tests — and never open or
 * read a file.
 */

/**
 * The ROOT-DETECTION half of the orient injection (and, since card
 * agent-hive-dispatch-warn-only--2026-08-21, of the dispatch-compose lint):
 * every registry cwd, basename idiom, absolute-path walk-up and assignee
 * cwd the searchText references, deepest-root-wins, docs-carrying
 * survivors only, ordered assignee-cwd-first then by first appearance.
 * Callers own fail-open handling; this function just detects.
 */
export function orientationRoots(
  searchText: string,
  assigneeCwd: string,
  registryCwds: string[],
  ...
  const kept = hits.filter((h) => !hits.some((o) => o !== h && o.root.indexOf(h.root + '/') === 0));
...
export function orientationBlock(
  ...
  try {
    const survivors = orientationRoots(searchText, assigneeCwd, registryCwds, probe);
    ...
    for (const root of shown) {
      const hasClaude = probe(root + '/CLAUDE.md');
      const hasAgents = probe(root + '/AGENTS.md');
      const docs =
        hasClaude && hasAgents
          ? provider === 'claude'
            ? 'CLAUDE.md'
          ...
          : hasClaude
            ? 'CLAUDE.md'
            : 'AGENTS.md';
      const graph = probe(root + '/graphify-out/graph.json')
        ? ' Knowledge graph present: run `graphify query "<question>"` before any grep.'
        : '';
      lines.push('- ' + root + ': read ' + docs + ' first and follow what it mandates.' + graph);
    }
    if (overflow > 0) {
      lines.push(
