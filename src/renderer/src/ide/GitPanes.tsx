      ...
      <div style={{ flex: 1, minHeight: 0, overflow: 'auto' }}>
        ...
        {commits.length >= page * 200 && (
          <div style={{ padding: '4px 12px' }}>
            <button style={smallBtn} onClick={() => setPage((p) => p + 1)}>
              load older…
            </button>
          </div>
        )}
          ...
          <div style={{ flex: 1, overflow: 'auto', minHeight: 0 }}>
            ...
            {files?.map((f) => (
              <FileRow
                ...
                f={f}
                onClick={() =>
                  onOpenRevDiff(`${selected.sha}^`, selected.sha, f.path, selected.shortSha)
                }
              />
            ))}
          </div>
...
export function ComparePane({
  ...
  const switchTo = async () => {
    ...
    if (
      !window.confirm(
        `Switch this repo to '${head}'?\n\nBlocked automatically if the tree is dirty or an agent is mid-run.`,
      )
    )
      return;
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', minHeight: 0, flex: 1 }}>
      ...
      >
        <div style={{ display: 'flex', gap: 4, alignItems: 'center' }}>
          <select
            ...
            style={sel}
            title="Base — the branch you're comparing against"
          >
            {branches.map((b) => (
          ...
          <button
            style={{ ...smallBtn, width: 22, justifyContent: 'center' }}
          ...
          <select
            ...
            style={sel}
            title="Compare — the branch whose changes you're viewing"
          >
            {branches.map((b) => (
              <option key={b} value={b}>
                {b}
        ...
        >
          {result && (
            <span
              title={`'${head}' is ${result.ahead} ahead and ${result.behind} behind '${base}'`}
            >
          ...
          <button
            style={{
              ...smallBtn,
              background: mode === 'three' ? 'var(--cth-sky-light)' : 'var(--cth-cream-100)',
            }}
          ...
          >
            {mode === 'three' ? 'since common ancestor' : 'literal difference'}
          </button>
          <span style={{ flex: 1 }} />
      ...
      >
        ...
        {result && result.files.length === 0 && !note && (
          <div style={noteStyle}>no differences</div>
        )}
        {result?.files.map((f) => (
          <FileRow
            key={f.path}
