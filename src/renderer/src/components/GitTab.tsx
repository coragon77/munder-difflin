  ...
  const refresh = async () => {
    ...
    try {
      ...
      if ('error' in b) setError(b.error);
        ...
        setBranch(b.current);
        setDetached(b.detached);
      }
      if ('error' in s) setError((prev) => prev ?? s.error);
      else setStatus(s);
      if (Array.isArray(l)) setLog(l);
      else if ('error' in l) setError((prev) => prev ?? l.error);
      if ('error' in br) setError((prev) => prev ?? br.error);
      else setBranches({ local: br.local, remote: br.remote });
      if ('error' in ab) {
        /* keep defaults */
      } else {
        setAhead(ab.ahead);
        setBehind(ab.behind);
        setUpstream(ab.upstream);
      }
    } finally {
      setLoading(false);
    }
  ...
  if (isRepo === false) {
    return (
      ...
      >
        Not a git repo.
        <br />
        Run <code style={{ fontFamily: 'var(--cth-font-mono)' }}>git init</code> in the agent's
        terminal.
      </div>
    );
  }
  ...
  return (
    ...
    >
      ...
      <div style={{ flex: 1, minHeight: 0, overflow: 'auto' }}>
        ...
        <Section title="status">
          {status && (
            <>
              <StatusGroup
                label="staged"
                entries={status.staged.map((e) => ({ ...e, code: e.index }))}
              />
              <StatusGroup
                label="changes"
                entries={status.unstaged.map((e) => ({ ...e, code: e.worktree }))}
              />
              <StatusGroup
                label="untracked"
                entries={status.untracked.map((p) => ({ path: p, code: '?' }))}
              />
        ...
        <Section title="log">
          {log.length > 0 ? (
            <CommitGraph commits={log} currentBranch={branch} />
          ) : (
            <div style={{ padding: 12, color: 'var(--cth-ink-500)', fontSize: 12 }}>
              no commits yet
            </div>
          )}
        </Section>
