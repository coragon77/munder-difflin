...
export function FullscreenFileEditor() {
  ...
  const matchedAgent = agents.find(
    (a) => fullscreenFilePath.startsWith(a.cwd + '/') || fullscreenFilePath === a.cwd,
  );
  ...
  const copyPath = () => {
    navigator.clipboard.writeText(fullscreenFilePath).catch(() => {
      /* noop */
    });
  };
    ...
    >
      ...
      >
        ...
        >
          ...
          <button onClick={openInIde} title="Open this file in the full IDE" style={chip}>
            open in IDE
          </button>
          <button onClick={() => setFullscreenFile(null)} title="Close (Esc)" style={chip}>
            ✕
          </button>
        </span>
      </div>
