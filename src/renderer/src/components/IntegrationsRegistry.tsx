...
function glyphFor(kind: string, label: string): { mono: string; bg: string } {
  return (
    GLYPH[kind] ?? { mono: label.replace(/[^A-Za-z0-9]/g, '').slice(0, 2) || '··', bg: '#6B5878' }
  );
}
...
function draftFromTemplate(t: IntegrationTemplate, now: number): Draft {
  return {
    isNew: true,
    id: slugify(t.idSuggestion || t.label),
    label: t.label,
    kind: t.kind,
    baseUrl: t.baseUrl,
    authType: t.authType,
...
export function IntegrationsRegistry() {
  ...
  const validate = (d: Draft): string | null => {
    ...
    if (d.kind === 'custom-rest') {
      ...
      if (
        ...
        return 'Base URL must be https:// (or http://127.0.0.1 / localhost for a local target).';
    }
    if (d.authType === 'header' && !/^[A-Za-z0-9-]{1,64}$/.test(d.authHeader.trim()))
      return 'Header name must be 1–64 chars of A–Z, 0–9, or "-".';
    return null;
  };
  ...
  const onTestRow = async (r: IntegrationRecordView) => {
    setTestingId(r.id);
    try {
      const res = await integrationsClient.test(r.id);
      setRowTest((m) => ({ ...m, [r.id]: res }));
    } catch {
      setRowTest((m) => ({ ...m, [r.id]: { ok: false, error: 'Test failed to run.' } }));
    } finally {
      setTestingId(null);
    }
  };
  const onTestCfg = async () => {
    ...
    setCfgTest(null);
    try {
      setCfgTest(await integrationsClient.test(draft.id));
    } catch {
      setCfgTest({ ok: false, error: 'Test failed to run.' });
    } finally {
      setTesting(false);
    }
  };
  ...
  if (view === 'gallery') {
    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
        <button type="button" onClick={goList} style={linkBtn}>
          ← Integrations
        </button>
        <div>
          <div style={{ ...dispLabel, marginBottom: 4 }}>Pick a template</div>
          <span style={subText}>
            Choose what you’re connecting. The template sets the defaults. Pick <b>Custom REST</b>{' '}
          ...
          {templates.map((t) => {
            ...
            return (
              <button
                key={t.idSuggestion}
                type="button"
                onClick={() => setPicked(t.idSuggestion)}
                style={{
                  display: 'flex',
                  alignItems: 'center',
                  gap: 10,
              ...
              >
                ...
                >
                  ...
                  <span style={hint}>
                    {t.secretHelp || (t.kind === 'custom-rest' ? 'Any HTTP API' : '')}
                  </span>
                </div>
              </button>
            );
        ...
        <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
          <PixelButton variant="secondary" size="sm" onClick={goList}>
            cancel
          </PixelButton>
          <PixelButton variant="primary" size="sm" onClick={continueFromGallery} disabled={!picked}>
            continue →
          </PixelButton>
        </div>
      </div>
  ...
  if (view === 'configure' && draft) {
    ...
    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
        <button type="button" onClick={draft.isNew ? startAdd : goList} style={linkBtn}>
          {draft.isNew ? '← Templates' : '← Integrations'}
        </button>
        ...
        >
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
            ...
            >
              {tpl?.label ?? draft.kind}
            </span>
            <span style={hint}>
              {needsSecret(draft.authType)
                ? `Needs a ${secretLabel.toLowerCase()}`
                : 'Public API — no secret needed'}
            </span>
          </div>
        </div>
          ...
          <input
            ...
            onChange={(e) =>
              patch({
                ...
                ...(draft.isNew ? { id: slugify(e.target.value) } : {}),
              })
            }
            placeholder={`e.g. ${tpl?.label ?? 'My API'} (prod)`}
            style={inputStyle}
          />
          <span style={hint}>
            Shown in your list. Id:{' '}
            <code style={{ fontFamily: 'var(--cth-font-mono)' }}>
              {slugify(draft.id || draft.label) || '—'}
            </code>
            {draft.isNew ? '' : ' (fixed)'}
          </span>
        </label>
        ...
        <label style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
          ...
          <input
            ...
            readOnly={draft.kind !== 'custom-rest'}
            style={{
              ...inputStyle,
              fontFamily: 'var(--cth-font-mono)',
              opacity: draft.kind !== 'custom-rest' ? 0.7 : 1,
            }}
          />
          {draft.kind !== 'custom-rest' && (
            <span style={hint}>Set by the {tpl?.label ?? 'preset'} template.</span>
          )}
        </label>
        ...
        {draft.kind === 'custom-rest' ? (
          <label style={{ display: 'flex', flexDirection: 'column', gap: 5, maxWidth: 260 }}>
            ...
            <select
              ...
              onChange={(e) => patch({ authType: e.target.value as IntegrationAuthType })}
        ...
        {draft.authType === 'header' && (
          <label style={{ display: 'flex', flexDirection: 'column', gap: 5, maxWidth: 320 }}>
            ...
            <span style={hint}>
              ...
              <code style={{ fontFamily: 'var(--cth-font-mono)' }}>
                {draft.authHeader.trim() || 'X-Header'}: &lt;secret&gt;
              </code>
              .
            </span>
          </label>
              ...
              <>
                <div style={{ display: 'flex', gap: 6 }}>
                  <input
                    ...
                    placeholder={`Paste your ${secretLabel.toLowerCase()}`}
                    autoComplete="off"
                    style={{ ...inputStyle, fontFamily: 'var(--cth-font-mono)' }}
                  />
                  <PixelButton
                ...
                <div
                  style={{
                    ...
                    padding: '7px 9px',
                    background: 'var(--cth-cream-100)',
                    boxShadow: 'inset 0 0 0 1px var(--cth-ink-100, var(--cth-ink-300))',
                    ...hint,
                  }}
                >
                  🔒&nbsp;
                  <span>
                    <b style={{ color: 'var(--cth-ink-700)' }}>Write-only.</b> Encrypted in the main
                    process and never shown again. To change it, paste a new key — the old one can’t
                    be read back.
                    {!draft.isNew && draft.hasSecret ? ' Leave blank to keep the saved key.' : ''}
                  </span>
                </div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            <span style={hint}>
              {isUsable
                ...
                : needsSecret(draft.authType) && !(draft.hasSecret || draft.secret.trim())
                  ? 'Add a secret and enable to make it available.'
                  : draft.enabled
                    ? 'Ready once saved.'
                    : 'Disabled — no worker can use it.'}
            </span>
          </div>
          <span style={hint}>
            v1 grants every enabled integration to all workers. Per-worker scoping is coming.
          </span>
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
          ...
          <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
            ...
            {cfgTest && (
              <span
                style={{
                  fontSize: 12,
                  color: cfgTest.ok ? 'var(--cth-mint-700, #1f7a4d)' : 'var(--cth-danger, #6E1423)',
                }}
              >
                {fmtTest(cfgTest)}
              </span>
          ...
          <span style={hint}>
            {draft.isNew
              ? 'Save the integration first, then test the live connection.'
              : 'Runs a live read-only probe against the base URL with the stored secret.'}
          </span>
        </div>
          ...
          {(err || note) && (
            <span
              style={{
                marginRight: 'auto',
                fontSize: 12,
                color: err ? 'var(--cth-danger, #6E1423)' : 'var(--cth-ink-500)',
              }}
            >
              {err || note}
            </span>
          )}
          <PixelButton variant="secondary" size="sm" onClick={goList} disabled={busy}>
            cancel
          </PixelButton>
          <PixelButton
            variant="primary"
            ...
            disabled={busy}
          >
            {busy ? '…' : draft.isNew ? 'Save integration' : 'Save changes'}
          </PixelButton>
        </div>
      </div>
    );
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
      ...
      >
        <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
          ...
          <span style={{ ...subText, maxWidth: 440 }}>
            Connect outside tools so your agents can read and act on them. Secrets are stored
            encrypted in the main process and never shown again.
          </span>
        </div>
        {records.length > 0 && (
          <PixelButton
      ...
      {records.length === 0 ? (
        ...
        >
          ...
          >
            No integrations yet. Connect GitHub or a custom REST API so your agents can use it.
          </p>
          <PixelButton
            variant="primary"
            size="sm"
        ...
        <>
          <span style={hint}>
            {records.length} integration{records.length === 1 ? '' : 's'} · {usableCount} available
            to workers
          </span>
                ...
                >
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
                    <span
                      style={{
                        ...hint,
                        color: usable(r) ? 'var(--cth-mint-700, #1f7a4d)' : 'var(--cth-ink-500)',
                      }}
                    >
