...
function eq(a: RGB, b: RGB): boolean {
...
function drawFace(
  ...
  if (lashes) {
    const lash: RGB = [54, 40, 48],
      glint: RGB = [252, 250, 248];
...
const styleShort: HairFn = (buf, color, skinBase, a) => {
  ...
  if (recede) {
    for (let y = 3; y < 6; y++)
      for (let x = 6; x < 12; x++) if (eq(rgbAt(buf, x, y), base)) set(buf, x, y, skinBase);
...
function drawHeavyFace(buf: Buf, skin: string): void {
  ...
  set(buf, 7, 17, s.sh);
  set(buf, 10, 17, s.sh); // crease shadow between chin + roll
}
