...
// archive, but flagged, so the Command Center can split one list into two
// shelves without ever falling out of step with each other.
//
...
test('removeArchivedAgent refuses while parked; recall + plain archive re-enables delete', () => {
  ...
  assert.equal(
    useStore.getState().archivedAgents.length,
    1,
    'a vacationer survives the delete call',
  );
  ...
  assert.deepEqual(
    ...
    [],
    'delete succeeds after recall + plain archive',
  );
});
