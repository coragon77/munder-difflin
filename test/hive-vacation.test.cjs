...
const vacationPool = (hive) => Object.entries(hive.registry().agents)
  .filter(([, a]) => !!a.vacation && !a.retired)
  .map(([id, a]) => ({ id, name: a.name, role: a.role ?? 'agent', cwd: a.cwd, parkedAt: a.vacationSince ?? null }));
...
test('parking sets archived + vacation and outlives the process', async (t) => {
  ...
  assert.equal(new HiveManager(() => home).isOnVacation('pam-1'), true,
...
test('a respawn IS the recall — it clears the flag and keeps the role', async (t) => {
  const { hive } = floor(t);
  await hive.ensureAgent({ id: 'pam-1', name: 'Pam', provider: 'claude', role: 'designer', cwd: '/tmp' });
  hive.setVacation('pam-1', true);
...
test('the retired and god are refused — vacation is for the living and the led', async (t) => {
  const { hive } = floor(t);
  await hive.ensureAgent({ id: 'intern-doomed', name: 'Ryan', provider: 'claude', role: 'intern', cwd: '/tmp' });
  await hive.ensureAgent({ id: 'michael', name: 'Michael', provider: 'claude', isGod: true, cwd: '/tmp' });
  hive.setRetired('intern-doomed', true);

  hive.setVacation('intern-doomed', true);
  hive.setVacation('michael', true);

  assert.equal(!!hive.registry().agents['intern-doomed'].vacation, false,
  ...
  assert.equal(!!hive.registry().agents['michael'].vacation, false, 'god runs the floor; god does not go on vacation');
});

test('a parked agent leaves the active roster for the fetchable pool', async (t) => {
  const { hive } = floor(t);
  await hive.ensureAgent({ id: 'pam-1', name: 'Pam', provider: 'claude', role: 'designer', cwd: '/repo' });
  ...
  assert.equal(pool[0].role, 'designer', 'god picks a fetchable by role, so it has to be in the pool');
