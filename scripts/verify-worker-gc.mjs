...
function gcSafe(wtPath, base) {
  ...
  if (gitOkExit(wtPath, 'diff', '--quiet', base, 'HEAD'))
    return { gc: true, detail: 'tree==base (squash)' };
...
git(repo, 'config', 'user.email', 'v@x');
...
git(wt('wC'), 'add', '-A');
git(wt('wC'), 'commit', '-qm', 'C1');
writeFileSync(join(wt('wC'), 'b.txt'), 'feature-C\nmore\n');
git(wt('wC'), 'add', '-A');
git(wt('wC'), 'commit', '-qm', 'C2');
...
console.log(
  `      (sanity: wC is ${cAhead} commits ahead of main yet tree-identical — ahead-count alone would never GC this)`,
);
