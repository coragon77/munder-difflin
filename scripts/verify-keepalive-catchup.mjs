...
const setT = (fn, delay) => {
  const id = nextId++;
  timers.set(id, { fireAt: nowMs + delay, fn, kind: 'timeout' });
  return id;
};
const setI = (fn, delay) => {
  const id = nextId++;
  timers.set(id, { fireAt: nowMs + delay, fn, kind: 'interval', intervalMs: delay });
  return id;
};
const clr = (id) => {
  timers.delete(id);
...
function advance(ms) {
  ...
  while (true) {
    ...
    for (const [id, t] of timers)
      if (t.fireAt <= target && (next === null || t.fireAt < timers.get(next).fireAt)) next = id;
    if (next === null) {
      nowMs = target;
      return;
    }
    ...
    nowMs = t.fireAt;
    if (t.kind === 'interval') t.fireAt += t.intervalMs;
...
function clearMissionTimers() {
  for (const e of missionTimers.values()) {
    if (e.timeout) clr(e.timeout);
    if (e.interval) clr(e.interval);
  }
  missionTimers.clear();
}
function syncMissions(m) {
  ...
  const fire = () => {
    fireCount++;
    m.lastFiredAt = nowMs;
    missionsUpdatedEmits++; /* mirrors liveWebContents().send('missions:updated') */
  };
  const remaining = Math.max(0, m.intervalMs - (nowMs - (m.lastFiredAt ?? 0))); // <- the exact line from index.ts
  const entry = {};
  entry.timeout = setT(() => {
    fire();
    entry.interval = setI(fire, m.intervalMs);
  }, remaining);
  missionTimers.set(m.id, entry);
}
const onSystemResume = (m) => {
  syncMissions(m);
}; // resume's scheduler re-arm (beats/keep-awake are not timer-fire-counted here)
...
ok(
  fireCount === 1,
  `overdue-by-5h fires exactly ONCE on resume (not 5x) -> fireCount=${fireCount}`,
);
ok(
  missionsUpdatedEmits === 1,
  `missions:updated emitted on the catch-up fire -> emits=${missionsUpdatedEmits}`,
);
advance(HOUR);
...
ok(
  fireCount === 0,
  `mission fired 10m ago does NOT fire early on resume -> fireCount=${fireCount}`,
);
advance(49 * 60 * 1000);
...
ok(
  fireCount === 1,
  `resume+unlock collapse to ONE catch-up fire (idempotent) -> fireCount=${fireCount}`,
);
...
const routeOnce = () => {
  ...
  delivered += n;
  return n;
}; // mirrors hive.routeOnce()
const startRouter = () => {
  if (routerTimer === null) routerTimer = setI(routeOnce, ROUTER_MS);
}; // idempotent, like hive.startRouter
const stopRouter = () => {
  if (routerTimer !== null) {
    clr(routerTimer);
    routerTimer = null;
...
const resumeOld = () => {
  /* BUG: scheduler/beats re-armed, router left frozen */
};
const resumeNew = () => {
  stopRouter();
  startRouter();
  routeOnce();
}; // the new onSystemResume router block
...
ok(
  delivered === 1,
  `awake: router drains a freshly-written outbox within one tick -> delivered=${delivered}`,
);
...
ok(
  delivered === 1,
  `pre-fix resume: backlog STAYS undelivered (reproduces the bug) -> delivered=${delivered}`,
);
...
ok(
  delivered === 6,
  `fixed resume: re-armed router drains subsequent mail -> delivered=${delivered}`,
);
