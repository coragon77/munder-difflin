...
> ✔ **Resolved (2026-08-22, Stefan):** deliberate — no kill-switch. Retirement is the tail of one bundled pipeline: agents work in worktrees and commit on branches, the restart-window watcher (armed by the AI, merging only while the app is down) lands them, and the next start retires what provably landed — bundled so "merging became mechanical instead of using orchestrator tokens" (Stefan). The pipeline's opt-in is *arming the merge watcher*: an unarmed watcher lands nothing, and retirement deletes only proven-landed branches, so it is a no-op in practice. A switch on retirement alone would mean "keep branches whose content is already on main", which the pushed `archive/<branch>` tags do anyway. The asymmetry with `autoParkIdle` is justified: auto-park acts on live state heuristically (idleness), retirement acts on proven facts. The only remote it ever touches is the origin of the checkout the app runs from (`retireLandedBranches(app.getAppPath())`, `index.ts:7663`); packaged builds abort at the mandatory fetch.
