...
- **2026-08-22 — post-close: code-defect digest from the coverage-gap round.** The six writer/reviewer pairs found 14 code defects while documenting. All are out of doc-run scope; Stefan cards the fixes (C2 precedent). Each is recorded in its owning doc next to its mechanism; this table is the card-from aggregation only.

| # | Defect | Recorded in |
|---|---|---|
| 1 | `burn5h` inflated for OTLP agents: `runBreakerBeat` appends cumulative snapshots (`index.ts:1674`), `CostSample` appends deltas (`hooks.ts:323`), `burn.ts:119` sums all rows with no diff step. Confirmed independently by writer and reviewer | `telemetry-collector.md` INTENT marker |
| 2 | Voice kill-confirm passes negations: `confirmAccepted("don't kill Jim", 'kill')` returns true (`norm()` strips the apostrophe, `\bkill\b` matches) | `realtime.md` gotcha + negation-screening INTENT marker |
| 3 | `telegram-reply.json` holds a live loopback bearer token written with bare `writeFileSync` — no `mode: 0o600`, unlike `slack-reply.json` and `integration-secrets.json` | `remote.md` INTENT marker + gotcha |
| 4 | `TelegramTrigger.stop()` abort is a no-op: `abort` declared at `telegram.ts:133`, never assigned; loop only ends via the `running` flag after the 25 s long poll | `remote.md` gotcha |
| 5 | pi agents spawn bare with no hive protocol seed: preset comment says "positional, like codex" but `positionalInitialPrompt` is never set, `hive.ts:1270-1272` falls through | `providers.md` VERIFY marker |
| 6 | IDE history pagination dead-ends at 500 commits: `git:logGraph` clamps `Math.min(500, …)`, `HistoryPane` gates "load older" on `commits.length >= page * 200`; the IPC `skip` offset is never passed | `ide.md:207` gotcha |
| 7 | IDE ✕ button discards unsaved edits without warning — only the Escape handler checks `anyDirtyRef` | `ide.md:213` gotcha |
| 8 | Slack webhook has no rate limit while the generic webhook that copied its shape (`0357ab0`, "modeled on the Slack webhook") carries a fixed window | `remote.md` INTENT marker |
| 9 | `groqChat()` has zero callers repo-wide; "VDE AI assist" (its commissioning feature, `11bdf12`) appears nowhere else in `src/` | `realtime.md` INTENT marker |
| 10 | Closing-time brief still instructs god about "the prep assistant", a role deleted in `6d0e092` | `lifecycle.md:127` gotcha |
| 11 | A voice dispatch registers no task card, so `detectCompletion`'s card-done branch can never fire for it — inbox-reply is the only working branch | `realtime.md` gotcha |
| 12 | `hive/docs/integrations-spec.md` is cited as "the contract" by three module headers and does not exist | `remote.md` VERIFY marker |
| 13 | `pricing.ts` header claims the table is "solely for the OFFLINE transcript reconciler" — stale since `fe49af8` (the hook plane prices through it; for non-Claude agents it writes the durable ledger `usd`) | `telemetry-collector.md` gotcha |
| 14 | No committed test for `IntegrationBroker` — `af22ac2` cites a "30/30 broker harness" absent from `test/` | `remote.md` gotcha (`fb8de90`) |
